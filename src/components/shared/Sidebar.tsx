...
export default function Sidebar({ user, role }: { user: any; role: string }) {
  ...
  // Sign out to /login — NextAuth will use NEXTAUTH_URL from env automatically
        ...
        {items.slice(0, 4).map(item => {
          ...
          return (
            <Link key={item.href} href={item.href}
              className={`flex-1 flex flex-col items-center justify-center py-2 gap-0.5 transition-colors ${active ? "text-indigo-400" : "text-zinc-500"}`}>
