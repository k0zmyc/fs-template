          ...
          {records.length > 0 && (
            <>
              <div className="sm:hidden space-y-2">
                {records.map(rec => (
                  <div key={rec.id} className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0">
                        {rec.imageUrl && <img src={rec.imageUrl} alt="" className="w-10 h-10 rounded-lg object-cover shrink-0" onError={e => (e.currentTarget.style.display="none")} />}
                        <div className="min-w-0">
                          <p className="font-medium text-zinc-100 truncate">{rec.title}</p>
                          <p className="text-xs text-zinc-600 mt-0.5">{new Date(rec.createdAt).toLocaleDateString()}</p>
                        </div>
                      </div>
                      {canEdit && (
                        <div className="flex gap-2 shrink-0">
                          <button onClick={() => openEdit(rec)} className="text-xs text-zinc-400 hover:text-zinc-200 px-2.5 py-1.5 rounded-lg border border-zinc-700 transition-colors">Edit</button>
